...
class GameLogic:
    ...
    def ai_turn(self):
        if self.movenum < 5:
            ...
            scoredict = self.simulate_future_turn(self.turns, self.board, 0, 0)
            maxvalue = max(scoredict.values())
            bestmoves = [key for key in scoredict if scoredict[key] == maxvalue]  # Finds turns with the highest scores.
    ...
    def simulate_future_turn(self, turns, board, score, depth):
        scoredict = {}
        depth += 1
        futureturnsarray = []
        ...
        for nextturn in self.check_columns(turns):
            futureturns = turns + str(nextturn)  # Concatenate turns with the newest turn.
            tempboard = copy.deepcopy(board)  # board copied by value.
            newboard, col, row = self.fill_board(futureturns,
                                                 tempboard)  # newboard is a board will all tokens matching futureturns.
            nextturnscore, winidentified = self.evaluate_score(newboard, int(col), int(row), depth,
                                                               playerorai)  # Checks score of the latest turn made.
            if playerorai == 1:
            ...
            for playerturnsubarray in futureturnsarray:
                if playerturnsubarray[2] == bestplayerscore:  # Ensures that only the best player move is branched out. This ensures that only the best player moves are considered (since the player is smart).
                    if depth >= self.maxdepth:  # self.maxdepth is reached, so stop.
                        ...
                        scoredict.update(
                            self.simulate_future_turn(playerturnsubarray[0], playerturnsubarray[1], bestplayerscore,
                                                      depth))
        else:
            for playerturnsubarray in futureturnsarray:
                if playerturnsubarray[3] and depth == 1:  # Immediate win is found, so move is returned automatically.
                    ...
                    scoredict.update(
                        self.simulate_future_turn(playerturnsubarray[0], playerturnsubarray[1], playerturnsubarray[2],
                                                  depth))
        return scoredict

    def fill_board(self, turns, board):  # MODIFIED IN THIS VERSION SO THAT THE ALL TOKENS ARE ADDED.
    ...
    def evaluate_score(self, board, col, row, depth, playerorai):
        winidentified = False
        rowcount, colcount, posdiagcount, negdiagcount = self.check_if_4_in_row(board, col, row, playerorai)
    ...
    def check_if_4_in_row_helper(self, board, col, colincrement, row, rowincrement, playerorai):
        counter = 0
    ...
    def check_if_4_in_row(self, board, col, row, playerorai):
        # Checks if a row of 4 successive tokens has been found.
        rowcount = self.check_if_4_in_row_helper(board, col, 1, row, 0, playerorai) + self.check_if_4_in_row_helper(
            board, col, -1, row, 0, playerorai) - 1
        # Checks if a column of 4 successive tokens has been found.
        colcount = self.check_if_4_in_row_helper(board, col, 0, row, 1, playerorai) + self.check_if_4_in_row_helper(
            board, col, 0, row, -1, playerorai) - 1
        # Checks if a diagonal line (with +ve gradient) of 4 successive tokens has been found.
        posdiagcount = self.check_if_4_in_row_helper(board, col, 1, row, -1,
                                                     playerorai) + self.check_if_4_in_row_helper(board, col, -1, row, 1,
                                                                                                 playerorai) - 1
        # Checks if a diagonal line (with -ve gradient) of 4 successive tokens has been found.
        negdiagcount = self.check_if_4_in_row_helper(board, col, -1, row, -1,
                                                     playerorai) + self.check_if_4_in_row_helper(board, col, 1, row, 1,
                                                                                                 playerorai) - 1
        return rowcount, colcount, posdiagcount, negdiagcount

    def is_game_over(self, col, row, playerorai):
        rowcount, colcount, posdiagcount, negdiagcount = self.check_if_4_in_row(self.board, col, row, playerorai)
