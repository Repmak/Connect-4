...
# AN IDENTIFIED WIN WILL STOP FUTURE TURNS FROM BEING GENERATED.
...
class gamelogic:
        ...
        if playerorai == 1:
            for playerturnsubarray in futureturnsarray:
                if playerturnsubarray[2] == bestplayerscore:  # Ensures that only the best player move is branched out. This ensures that only the best player moves are considered (since the player is smart).
                    if playerturnsubarray[3] or depth >= self.maxdepth:  # Win is found, so future turns are not generated since the game is over.
                    ...
                    else:  # self.maxdepth is not reached, and no win is found, so future turns are generated.
                        print(playerturnsubarray[0])
                        print(playerturnsubarray[2])
                        print('DEPTH: ' + str(depth))
            ...
            for playerturnsubarray in futureturnsarray:
                ...
                elif playerturnsubarray[3] or depth >= self.maxdepth:  # Win is found, so future turns are not generated since the game is over.
                    print(playerturnsubarray[0])
                    print(playerturnsubarray[2])
                    print('DEPTH: ' + str(depth))
                    print('AI TURN ADDED')
    ...
    def evaluatescore(self, board, col, row, depth, playerorai):
        ...
        if rowcount >= 4 or colcount >= 4 or posdiagcount >= 4 or negdiagcount >= 4:
            winidentified = True
            score += 1000000 / (depth * playerorai ** 3)
            # score += (1000 * math.exp(-0.05 * depth))
