...
class Connect4Window(pyglet.window.Window):
    ...
    def draw_winner(self):
        if self.gamelogic.gamestate == 1:
            winlabel = pyglet.text.Label(text='Player wins!', font_name='microsoft yi baiti', font_size=35, color=(247, 40, 40, 255), x=730, y=50)
        elif self.gamelogic.gamestate == 2:
            winlabel = pyglet.text.Label(text='AI wins!', font_name='microsoft yi baiti', font_size=35, color=(255, 230, 0, 255), x=730, y=50)
        else:
            winlabel = pyglet.text.Label(text='This round is a draw.', font_name='microsoft yi baiti', font_size=35, color=(110, 110, 110, 255), x=730, y=50)
        winlabel.draw()

    def on_key_press(self, symbol, modifiers):
        if (symbol == key.D or symbol == key.RIGHT) and self.gamelogic.gamestate == 0:
            self.gamelogic.move_token(1)
    ...
    def update(self, dt):
        if not self.isplayerturn and self.gamelogic.gamestate == 0:
            ...
            self.gamelogic.ai_turn()  # Run AI turn.
